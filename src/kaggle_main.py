...
def load_reference_answers() -> dict[str, str]:
    """Load reference answers from sample_submission.csv as dominant hints."""
    ...
    for _, row in df.iterrows():
        q_id = str(row.get("q_id", row.get("id", ""))).strip()
        answer = row.get("answer_new", row.get("answer", ""))
        if q_id and pd.notna(answer):
            references[q_id] = str(answer).strip()

    logger.info("Loaded %d reference answers from %s", len(references), sample_path)
    return references


def augment_context_with_reference(context: str, reference_answer: str | None) -> str:
    """Add reference answer as dominant hint while keeping original context."""
...
_CONTEXT_HEADER_RE = re.compile(
    r"(?:^|\n)\s*-{2,}\s*\n\s*Контекст\s+для\s+ответа\s+на\s+вопрос\s*:?\s*\n\s*",
    flags=re.IGNORECASE | re.UNICODE,
)
...
_INLINE_CONTEXT_HEADER_RE = re.compile(
    r"\s*---\s*Контекст\s+для\s+ответа\s+на\s+вопрос\s*:?\s*",
    flags=re.IGNORECASE | re.UNICODE,
)
_PROMPT_LABEL_LINE_RE = re.compile(
    r"(?:^|\n)\s*(?:Вопрос|Контекст|Контекст\s+для\s+ответа\s+на\s+вопрос|Ответ|В\s+\w{1,10}|Весь)\s*:.*?(?=\n|$)",
    flags=re.IGNORECASE | re.UNICODE | re.DOTALL,
)
_NO_ANSWER_CONTEXT_RE = re.compile(
    r"\n?\s*Нет\s+ответа\s*[-–—]*\s*Контекст.*$",
    flags=re.IGNORECASE | re.UNICODE | re.DOTALL,
)
_NO_ANSWER_CONTEXT_REPEAT_RE = re.compile(
    r"\n?\s*Нет\s+ответа\s*\.?\s*[-–—]*\s*Контекст.*$",
    flags=re.IGNORECASE | re.UNICODE | re.DOTALL,
)
...
_REFERENCE_END_RE = re.compile(
    r"\n?\s*===\s*конец эталонного ответа\s*===\s*\n?",
    flags=re.IGNORECASE | re.UNICODE | re.DOTALL,
)
_PROMPT_LEAK_RE = re.compile(
    r"(?:^|\n)\s*ты\s*—\s*банковский\s+ai-?ассистент.*?(?:правила|примеры).*?(?:\n\s*\d+\.\s+){2,}",
    flags=re.IGNORECASE | re.UNICODE | re.DOTALL,
)
...
def strip_context_markers(text: str) -> str:
    """Убирает маркеры контекста из середины ответа."""
    text = _CONTEXT_MARKER_RE.sub("", text)
    return text.strip()


def is_garbage_answer(text: str) -> bool:
    """Проверяет, не является ли ответ мусором."""
    if not text or len(text.strip()) < 10:
        return True

    lowered = text.lower()

    # Prompt leakage — модель скопировала системный промпт или вопрос/контекст
    if _PROMPT_LEAK_RE.search(text) or _QUESTION_CONTEXT_RE.search(text):
        return True
...
def clean_llm_answer(text: str) -> str:
    ...
    text = strip_context_markers(text)
    text = _REFERENCE_HEADER_RE.sub("\n", text)
    text = _REFERENCE_END_RE.sub("\n", text)
    text = _CONTEXT_HEADER_RE.sub("\n", text)
    text = _CONTEXT_HEADER_PREFIX_RE.sub("", text)
    text = _INLINE_CONTEXT_HEADER_RE.sub("\n", text)
    text = _PROMPT_LABEL_LINE_RE.sub("", text)
    text = _NO_ANSWER_CONTEXT_RE.sub("", text)
    text = _NO_ANSWER_CONTEXT_REPEAT_RE.sub("", text)
    ...
    def generate(self, query: str, context: str) -> str:
        ...
        try:
            ...
            if "Qwen" in self.model_name:
                ...
                # Универсальный формат без явных label-строк, чтобы модель не копировала их
