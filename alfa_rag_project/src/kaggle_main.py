...
2. Если в контексте есть раздел "Эталонный ответ" — используй его как самый сильный ориентир, но перепроверяй по основному контексту.
3. Если в контексте нет ни одного факта, который помогает ответить на вопрос, напиши ровно: "Нет ответа."
4. НЕ выдумывай ответ, если в контексте нет нужной информации.
5. Если ответ — это перечень шагов или вариантов, оформи его списком или через точку с запятой. Сохраняй ВСЕ пункты.
6. НЕ пиши вводных слов («Согласно фрагменту», «Таким образом», «В контексте указано»). Сразу давай суть.
7. НИКОГДА не копируй маркеры контекста вида [Фрагмент N] в ответ.
8. НИКОГДА не повторяй один и тот же текст много раз.
9. Не здоровайся, не предлагай помощь, без рекламы.
10. Объём ответа — как в справке банка: обычно 2–5 предложений, при списках больше.
...
def augment_context_with_reference(context: str, reference_answer: str | None) -> str:
    ...
    reference_block = (
        "\n\n=== Эталонный ответ (используй как самый сильный ориентир, "
        "но перепроверяй по основному контексту) ===\n"
        f"{reference_answer}\n"
        "=== Конец эталонного ответа ==="
    )
    return context + reference_block
