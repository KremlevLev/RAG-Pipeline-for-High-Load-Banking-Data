"""
Test version of OpenRouter RAG pipeline.
Processes only 100-150 questions for adjudication/testing.
"""
...
def run_pipeline(
    ...
    """
    Запускает RAG pipeline через OpenRouter (тестовая версия).
    
    Обрабатывает только max_questions вопросов для тестирования.
    ...
    Args:
        ...
        max_questions: Максимальное количество вопросов для обработки (по умолчанию 150)
    """
    # ── Индекс ────────────────────────────────────────────────
    ...
    total = min(len(questions_df), max_questions)
    logger.info(f"Processing {total} questions (test mode)")
    ...
    # Сохраняем в отдельный файл для теста
    test_submission_path = SUBMISSION_CSV.parent / "submission_test.csv"
    pd.DataFrame(results).to_csv(test_submission_path, index=False)
    logger.info("Test results: %s", test_submission_path)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="OpenRouter RAG Pipeline (Test Mode)")
    ...
    parser.add_argument(
        ...
        type=int,
        default=150,
        help="Maximum questions to process (default: 150 for testing)",
    )
